(function (window) {
	...
	init: function(query, options) {
		...
		carouselElement.ondragstart = function() { return false; };
		...
		offset = target = 0;
		images = [];
		item_width = parseInt(options["itemWidth"]);
		...
		function scroll(x) {
                    var i=0, half, delta, dir, tween, el, alignment, tweenedOpacity, zTranslation;
			...
			var sti = alignment +
				...
				' translateY(' + (dir * options.shift * tween * i) + 'px)' +
